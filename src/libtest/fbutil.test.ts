...
user: I am`;
  ...
  describe('Image',async function (){
      it('http', async function (){
          const result = await Fbutil.parse(`user: Hello Hello![](http://image)`, detail, dir,false);
          assert.deepEqual(result, [
              { role: 'user', content: [
                  { type: 'text',
                    text: 'Hello Hello'},
      ...
      it('local base64', async function (){
          const result = await Fbutil.parse(`user: Hello Hello![](fbehrens.jpeg)`, detail, dir,false);
